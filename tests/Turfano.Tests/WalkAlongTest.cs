...
public class WalkAlongTest
{
    ...
    public async Task WalkAlong_ToSecondSegment()
    {
        ...
        var segments = Turf.GetSegments(circuit).ToArray();

        // Calculate the total length and segment lengths for precise position verification
        var totalLength = Turf.GetLength(circuit);
        var firstSegmentLength = Turf.Distance(circuit.Coordinates[0], circuit.Coordinates[1]);
        ...
        var expectedSegmentProgress =
            ...
            / Turf.Distance(circuit.Coordinates[1], circuit.Coordinates[2]).Kilometers;
        var expectedX = expectedSegmentProgress * QuarterSide;
        var expectedY = QuarterSide;

        var point = Turf.WalkAlong(circuit, distanceToWalk);
    ...
    public async Task WalkAlong_Loop()
    {
        ...
        var length = Turf.GetLength(circuit);
        var point = Turf.WalkAlong(circuit, length);
        var tolerance = 1e-10;
        ...
        var halfPoint = Turf.WalkAlong(circuit, halfLength);
    ...
    public async Task WalkAlong_OneAndHalfLoop()
    {
        ...
        var circuitLength = Turf.GetLength(circuit);
        ...
        var distFromStartToSegment = Turf.Distance(
            circuit.Coordinates[2],
            circuit.Coordinates[3]
        ).Meters;
        var progressOnSegment =
            (halfwayDistance - distFromStartToSegment)
            / Turf.Distance(circuit.Coordinates[3], circuit.Coordinates[4]).Meters;

        // Make sure we're properly tracking multiple loops
        var oneLoopPoint = Turf.WalkAlong(circuit, circuitLength);
    ...
    public async Task WalkAlong_NegativeDistance()
    {
        ...
        var point = Turf.WalkAlong(circuit, negativeDistance);
        var tolerance = 1e-10;
        ...
        var fullLength = Turf.GetLength(circuit);
        var fullNegativePoint = Turf.WalkAlong(circuit, fullLength * -1);
