...
- [ ] T001 Criar `docs/nts-evaluation.md` com o esqueleto das 6 seções exigidas em
...
- [ ] T002 [P] [US2] Catalogar todas as funções de `Turf` (lendo `src/Turfano/*.cs`):
  classificação `nts-wrapper`/`own`/`naive` + visibilidade `public`/`internal`. Preencher
  a **Seção 1** de `docs/nts-evaluation.md`.
- [ ] T003 [US2] Criar o harness de ground-truth do TurfJS em `reference/` (script Bun +
...
- [ ] T004 [US2] Criar um runner C# descartável (ex.: `reference/`-paralelo ou um
  pequeno console) que emite as saídas do **Turfano** para as mesmas fixtures.
- [ ] T005 [US2] Comparar as saídas conforme o critério de `research.md` (Decisão 2:
...
- [ ] T006 [US1] Para cada op pesada/ingênua (`union, difference, intersect, dissolve,
...
- [ ] T007 [US1] Preencher a **Seção 3** (matriz op-a-op) com a `decision`
...
- [ ] T008 [P] [US3] Adicionar protótipo **descartável** em
...
- [ ] T009 [US3] Rodar `dotnet run -c Release --project benchmark/TimeAndMemoryUsage` e
...
- [ ] T010 [P] [US4] Inventariar UnitsNet (`grep -rn 'UnitsNet\|Length\|Angle\|Area'
...
- [ ] T011 Escrever a **Seção 6** (recomendação final por operação: manter/remover NTS +
  conclusão sobre adotar tipos próprios), consolidando as Seções 3 e 4.
- [ ] T012 Verificar SC-006: `git diff --stat main -- src/Turfano` vazio e
  `dotnet run --project tests/Turfano.Tests -c Debug` = 156/0; registrar no doc.
- [ ] T013 [P] Remover (ou marcar claramente como fixtures de avaliação, não-produção) os
  scripts efêmeros em `reference/` e os protótipos em `benchmark/`, conforme a decisão.
- [ ] T014 Atualizar `plans/turfjs-parity-redesign.md`: Fase 2 → `Complete` + Phase
