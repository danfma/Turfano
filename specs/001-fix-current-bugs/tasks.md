...
- [X] T001 Confirmar baseline verde no estado pré-patch e registrar a contagem:
...
**Independent Test**: rodar `RhumbBearingTests` — a âncora `-170.29°` e os casos > 180°/
antimeridiano passam.

### Tests for User Story 1 ⚠️ (escrever PRIMEIRO, confirmar que FALHAM)

- [X] T002 [P] [US1] Criar `tests/Turfano.Tests/RhumbBearingTests.cs` com os casos:
  (a) âncora `RhumbBearing((-75.343,39.984),(-75.534,39.123)) ≈ -170.29°` `.Within(0.1)`
  (valor real do TurfJS; `9.71°` é o sentido inverso e era o valor do bug);
...
- [X] T003 [US1] Rodar `dotnet run --project tests/Turfano.Tests -c Debug -- --treenode-filter "/*/*/RhumbBearingTests/*"`
  e CONFIRMAR FALHA (red) no código atual; registrar a saída como evidência do bug.

### Implementation for User Story 1

- [X] T004 [US1] Corrigir `src/Turfano/Angles.cs`: `TwoPi = Angle.FromRadians(2 * Math.PI)`
  (era `Math.PI`). Fix compartilhado — também habilita a US3.
- [X] T005 [US1] Validar **sem editar** `src/Turfano/Turf.RhumbBearing.cs`: confirmar que
...
- [X] T006 [US1] Rodar `RhumbBearingTests` (verde) e a suíte completa
...
- [X] T007 [P] [US2] Criar `tests/Turfano.Tests/TransformScaleTests.cs` com os casos:
...
- [X] T008 [US2] Rodar `dotnet run --project tests/Turfano.Tests -c Debug -- --treenode-filter "/*/*/TransformScaleTests/*"`
  e CONFIRMAR FALHA do caso padrão (red; evidência do bug — SC-005).

### Implementation for User Story 2

- [X] T009 [US2] Corrigir `src/Turfano/Turf.TransformScale.cs` (linha ~49):
  `var scaledY = dy * (options.FactorY ?? factor);` (parentização).
- [X] T010 [US2] FR-009 — varrer o padrão de precedência:
...
- [X] T011 [US2] Rodar `TransformScaleTests` (verde) e a suíte completa para não-regressão.
...
- [X] T012 [P] [US3] Criar `tests/Turfano.Tests/GetAngleTests.cs`: ângulo base de um trio
  conhecido (sem `Explementary`) e `Explementary = 360° − θ`; borda em `0°`/`360°`.
- [X] T013 [US3] Rodar `dotnet run --project tests/Turfano.Tests -c Debug -- --treenode-filter "/*/*/GetAngleTests/*"`
...
- [X] T015 Verificação final (SC-004): `dotnet build Turfano.slnx -c Debug` (0 erros em
  todos os TFMs) + `dotnet run --project tests/Turfano.Tests -c Debug`
  (0 falhas; total = 146 + novos).
- [X] T016 Atualizar `plans/turfjs-parity-redesign.md`: marcar os checkboxes da Fase 1,
